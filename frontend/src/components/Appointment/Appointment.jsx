...
function Appointment() {
    ...
    function isAppointmentDisabled(hours, wholeMonth) {
        if (
            ...
            hours.slice(0, 2) <=
                hoursArray.map((hour) => parseInt(hour.slice(0, 2)))
        ) {
            return true;
        }
        if (
                    ...
                    <div className="appointment--container">
                        {!displayNextWeek &&
                            wholeWeekArrayNextWeek.map((wholeMonth, index) => (
                                <div
                                    className="appointment--fulldate"
                                    key={index}
                                >
                                    <h2 className="appointment--month">
                                        {wholeMonth.month}
                                    </h2>
                                    <h1 className="appointment--date">
                                        {wholeMonth.days}
                                    </h1>
                                    <h2 className="appointment--day">
                                        {wholeMonth.weekdays}
                                    </h2>
                                    {hoursArr.map((hours, i) => (
                                        <Link
                                            className={`appointment--hours ${
                                                isBooked(hours, wholeMonth) &&
                                                "admin-disable"
                                            }`}
                                            ref={(element) =>
